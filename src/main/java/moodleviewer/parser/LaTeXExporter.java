...
public class LaTeXExporter {
    ...
    public static void exportToLaTeX(Category rootCategory, File file, boolean showAnswers) throws IOException {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(file, java.nio.charset.StandardCharsets.UTF_8))) {
            ...
            // Traducción aplicada al título del documento
            writer.write("\\title{" + I18n.get("latex.title") + "}\n");
    ...
    /**
     * Compila un archivo .tex a .pdf ejecutando pdflatex en el sistema y lo abre.
     * @param texFile El archivo LaTeX generado previamente.
     * @throws IOException Si ocurre un error al ejecutar el comando o no se encuentra el PDF.
     * @throws InterruptedException Si el proceso de compilación es interrumpido.
     */
    public static void compileAndOpenPDF(File texFile) throws IOException, InterruptedException {
        // Archivo temporal donde tiraremos todo el texto que escupe la consola 
        // para que Java no se sature y se cuelgue.
        File consoleLog = new File(texFile.getParentFile(), "salida_consola_java.txt");

        // PRIMERA PASADA: Modo borrador (-draftmode). 
        ProcessBuilder pb1 = new ProcessBuilder("pdflatex", "-interaction=nonstopmode", "-draftmode", texFile.getName());
        pb1.directory(texFile.getParentFile()); 
        pb1.redirectErrorStream(true);      // Junta errores y mensajes normales
        pb1.redirectOutput(consoleLog);     // ¡LA MAGIA! Envía el texto aquí para no bloquearse
        pb1.start().waitFor();
        
        // SEGUNDA PASADA: Normal.
        ProcessBuilder pb2 = new ProcessBuilder("pdflatex", "-interaction=nonstopmode", texFile.getName());
        pb2.directory(texFile.getParentFile()); 
        pb2.redirectErrorStream(true);
        pb2.redirectOutput(consoleLog);
        pb2.start().waitFor();

        File pdfFile = new File(texFile.getParentFile(), texFile.getName().replace(".tex", ".pdf"));
        
        if (pdfFile.exists() && java.awt.Desktop.isDesktopSupported()) {
            java.awt.Desktop.getDesktop().open(pdfFile);
        } else {
            throw new IOException(I18n.get("file.err.pdf"));
        }
    }
}